    ...
    try:
        ...
        while (bufferChecking):
            ...
            for i in range(bytesInBuffer):
                ...
                #if isX10RF(rxBuffer):
                #	processX10RfMsg(rxBuffer)
                #elif (isDM10(rxBuffer):
                #	processDM10Msg(rxBuffer)
                
                # Check if is a SECURITY message
                if (sensorType := isSecurity(subBuffer)) != RFXCOM_SECURITY_MSG_UNKNOWN:
                    msgFound = True
...
def publishToMqtt(sensorData):
    ...
    if sensorType == RFXCOM_SENSOR_SECURITY or sensorType == RFXCOM_SENSOR_OREGON or sensorType == RFXCOM_SENSOR_RFXCOM:
        addr = sensorData[1]
...
### MAIN ### 
client = mqtt.Client("P2") # must be unique on MQTT network
